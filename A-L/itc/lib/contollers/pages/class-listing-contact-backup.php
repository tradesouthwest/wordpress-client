 ...
 * If Views Addon plugin contact form data lost, use this -backup.php
 * to copy into class-listing-contact.php to recover (line 305+)
 * @since 3.4
...
class WPBDP__Views__Listing_Contact extends WPBDP__View {
    ...
    public function dispatch() {
        ...
        if ( $email->send() ) {
            $html .= wpbdp_render_msg( _x( 'Your message has been sent. Please wait for an administrator to validate your request.', 'contact-message', 'business-directory-plugin' ) );
        ...
        //?wpbdp_view=listing_contact&listing_id=22998
        $html .= sprintf( '<p><a href="%s">%s</a></p>', 
                    get_permalink( $listing_id ), 
                    _x( 'Return to listing without file views.', 'contact-message', 'business-directory-plugin' )
                );
        
        if ( function_exists( 'views_addon_render_judges_access_link' ) ) {
            $access_key = get_post_meta( $listing_id, '_wpbdp[access_key]', true );
        $html .= sprintf( '<p><a href="%s">%s</a></p>',
                    rtrim(get_permalink( $listing_id ), '/') . '&access_key=' . $access_key, 
                    _x( 'Return to listing with file views.', 'contact-message', 'business-directory-plugin' )
            );
        }
