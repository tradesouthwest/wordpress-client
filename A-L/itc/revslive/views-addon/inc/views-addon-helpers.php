...
// #a4
add_action('views_addon_itc_contact_review', 'views_addon_render_itc_contact_review' );
...
function views_addon_assigned_protected_body_class( $classes ) {
    ...
    $assigned_judge_email = get_post_meta( $post_id, $custom_field, true ); 
...
function views_addon_inclusive_protected_post_type($classes){
    ...
    $user = wp_get_current_user();
    $allowed_roles = array( 'administrator', 'author', 'editor', 'pdtsw_mediator' );
...
/** id=a4
 * Add form to single view listing template
 * @uses CF7 form  [contact-form-7 id="22710" title="Contact form 2"]
 */
function views_addon_render_itc_contact_review(){
    $htm = '';
    $shrtcode    = ( empty( get_option('views_addon_options')['views_addon_evalcf7_formused'] )) 
                     ? '' : get_option('views_addon_options')['views_addon_evalcf7_formused'];
    $eval_notice = ( empty( get_option('views_addon_options')['views_addon_eval_notice'] )) 
                     ? '' : get_option('views_addon_options')['views_addon_eval_notice'];
    if ( views_addon_check_for_assigned_judge() ){
        $htm .= 
        '<div class="clearfix"></div>
        <div class="listing-details reved">
        <section><h4>Review Play Form</h4>
        <p class="eval-notice">' . esc_html( $eval_notice ) . '</p>'
        . do_shortcode( $shrtcode ) 
        . '</section>
        </div>';
