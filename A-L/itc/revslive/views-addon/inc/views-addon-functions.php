...
// #a7
add_action( 'views_addon_assign_judge_oncontact', 
    'views_addon_assign_judge_oncontact_autoassign', 10, 2); 
/* =========== Filter Hooks =========== */
// #f1
 ...
 */ 
function views_addon_assign_judge_oncontact_autoassign( 
                    $vatsw_listing_id, 
                    $vatsw_user_id )
{ 
    if ( ! is_user_logged_in() ) {
    ...
    $custom_field        = '_wpbdp[fields][23]'; 
    $assigned_judge_email = get_post_meta( $post->ID, $custom_field, true ); 
    $emailto   = ( empty( get_option('views_addon_options')['views_addon_request_emails'] )) 
        ...
        $assign_judge = $new_judge->user_email;
        $subscribers = array( $emailto );
	    $subject     = 'A new Judge has beed assigned!';
        ...
        $assign_judge_token = sanitize_text_field( $assign_judge );
        $success = wp_mail( $emailto, $subject, $message );
        if( $success ) {
            update_post_meta( $post->ID, $custom_field, $assign_judge_token );
        } else {
        echo 'email not sending. please contact Larry.' . $assigned_judge_email;
        }  
 ...
 * Auto-assign judge to listing.
 */
function views_addon_deassign_judge_autoassign( $vatsw_listing_id, 
                                                $vatsw_user_id )
                                                { 
// on init check assigned plays

// check against start_time plus 24 hrs.

// if email form not_sent email first then reassign
return false;
}
