...
class WPBDP_Email {
    ...
    //$attachments = array( WP_CONTENT_DIR . '/uploads/
    private function get_attachments() {
        ...
        $listing_id = wpbdp_get_var( array( 'param' => 'listing_id', 
                                            ...
                                            'request' );
        $attachments = [];
        $attached = WPBDP_ListingAttachmentsModule::get_attachments($listing_id);
        
        foreach ( $attached as $a ) {
            
            $path = ( !isset( $a['file_']) ) ? '' : $a['file_'];
            $attachments[] = WP_CONTENT_DIR . '/uploads/' . $path;
        } 
    ...
    public function send() {
        ...
        $result = wp_mail( $this->get_recipients(), 
