...
?><div class="<?php echo esc_attr( implode( ' ', $gallery_classes ) ); ?>"><?php
	...
	if ( has_post_thumbnail( $product_id ) ) {
		...
		$image_data    = wp_get_attachment_image_src( $image_post_id, 'thumbnail' );
		$image_link    = $image_data[ 0 ]; 
		$product_link  = get_permalink( $product_id ); // url of single product
		$image_norel   = 'product_url';                     // added norel ambiguous
