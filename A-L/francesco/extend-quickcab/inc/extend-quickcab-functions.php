...
function codeable_larry_custom_user_profile_fields( $user ) {
    ...
    <table class="form-table">
        ...
        <tr>
            <th><label for="school_name"  title="school name"><?php esc_html_e( 'Nome scuola', 'hello-elementor' ); ?></label></th>
            <td>
...
function extend_quickcab_checkout_field_process() {
    ...
    if ( ! $_POST['school_name'] )
        wc_add_notice( __( 'Please enter school name field.' ), 'error' );
}
/**
...
function extend_quickcab_checkout_field_display_admin_order_meta($order){
    ...
    echo '<p><strong>'.__('Istituto comprensivo').':</strong> ' . get_post_meta( $order->id, 'comprehensive_school', true ) . '</p>';
    echo '<p><strong>'.__('Nome scuola').':</strong> ' . get_post_meta( $order->id, 'school_name', true ) . '</p>';
    echo '<p><strong>'.__('Istituzione scolastica').':</strong> ' . get_post_meta( $order->id, 'school_type', true ) . '</p>';
    echo '<p><strong>'.__('').':</strong> ' . get_post_meta( $order->id, 'cancel_message', true ) . '</p>';
