          ...
          <tr id="mepr-subscription-row-<?php echo $s->id; ?>" class="mepr-subscription-row <?php echo (isset($alt) && !$alt)?'mepr-alt-row':''; ?>">
            ...
            <td data-label="<?php _ex('Actions', 'ui', 'memberpress'); ?>">
                <div class="mepr-account-actions">
                  <?php
                  if($txn != false && $txn instanceof MeprTransaction && ($txn->is_sub_account() 
                      || $txn->txn_type == MeprTransaction::$fallback_str)) { echo '--';
                  }
                  else {
                    if( $is_sub && $pm instanceof MeprBaseRealGateway &&
                      ...
                      if(!$subscription->in_grace_period()) { 
                        //Don't let people change shiz until a payment has come through yo
                    ...
                    elseif(!$is_sub && !empty($prd->ID)) {  
                      if($prd->is_renewable() && $prd->is_renewal()) {
                        ...
                        <?php      
                      } //ends check for renewal product ln. 184
...
if( function_exists( 'meprprorata_get_current_subscription_links' ) ) :  

  $parse_url = meprprorata_get_current_subscription_links($prd->ID); 
  if( $parse_url ) { 
  ?>

  <a href="<?php echo esc_url( $parse_url ); ?>" class="mepr-account-row-action mepr-account-renew"><?php _ex('Renew + Fee', 'ui', 'memberpress'); ?></a> 
...
// Ends **************** added to validate parent sub ************************                        
// Remember to update this file AROUND addon        
                      ...
                      if($prd->can_you_buy_me()) {
                        if($group !== false) {
                          ...
                          if(!$sub_in_group && !$life_in_group) { 
                            //$prd is in group, but user has no other active subs in this group, so let's show the change plan option
          ...
          <td class="mppr-text mppr-td">
            <?php //From plugin MeprProRata <th>+<td>added to fit
               
        if( function_exists( 'meprprorata_get_current_day_number' ) ) : 
          ?><span><?php 

              meprprorata_get_current_day_number( $prd->ID );
