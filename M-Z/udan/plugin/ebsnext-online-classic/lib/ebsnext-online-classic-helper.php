 ...
 *
 * All use of the payment modules happens at your own risk.
 *
 * @author    Ebsnext Online
 * @copyright Ebsnext Online (https://nextpay.com) 
 * @license   Ebsnext Online
 */
class Ebsnext_Online_Classic_Helper
{
    const ROUND_UP = "round_up";
    const ROUND_DOWN = "round_down";
    const ROUND_DEFAULT = "round_default";
    const EBSNEXT_ONLINE_CLASSIC_TRANSACTION_ID_LEGACY = 'Transaction ID';
    const EBSNEXT_ONLINE_CLASSIC_SUBSCRIPTION_ID = 'ebsnext_online_classic_subscription_id';
    const EBSNEXT_ONLINE_CLASSIC_SUBSCRIPTION_ID_LEGACY = 'Subscription ID';
    const EBSNEXT_ONLINE_CLASSIC_PAYMENT_TYPE_ID = 'Payment Type ID';
    const EBSNEXT_ONLINE_CLASSIC_STATUS_MESSAGES = 'ebsnext_online_classic_status_messages';
    const EBSNEXT_ONLINE_CLASSIC_STATUS_MESSAGES_KEEP_FOR_POST = 'ebsnext_online_classic_status_messages_keep_for_post';
    const ERROR = 'error';
    ...
    public static function create_admin_debug_section() {
        $documentation_link = 'http://woocommerce.com';
        ...
        $html .= sprintf( '<a id="boclassic-admin-log" class="button" href="%s" target="_blank">View debug logs</a>', self::EBSNEXT_instance()->get_boclassic_logger()->get_admin_link() );
    ...
    /**
     * Get the Ebsnext Online Classic Subscription id from the order
     *
     * @param WC_Subscription $subscription
     */
    public static function get_ebsnext_online_classic_subscription_id( $subscription ) {
        $subscription_id = self::is_woocommerce_3() ? $subscription->get_id() : $subscription->id;
        $ebsnext_subscription_id = get_post_meta( $subscription_id, self::EBSNEXT_ONLINE_CLASSIC_SUBSCRIPTION_ID, true );

        //For Legacy
        if( empty( $ebsnext_subscription_id ) ) {
            $parent_order_id = self::is_woocommerce_3() ? $subscription->get_parent_id() : $subscription->parent_id;
            $ebsnext_subscription_id = get_post_meta( $parent_order_id, self::EBSNEXT_ONLINE_CLASSIC_SUBSCRIPTION_ID_LEGACY, true );
            if( !empty( $ebsnext_subscription_id ) ) {
                //Transform Legacy to new standards
                update_post_meta( $subscription_id, self::EBSNEXT_ONLINE_CLASSIC_SUBSCRIPTION_ID, $ebsnext_subscription_id );
                delete_post_meta( $parent_order_id, self::EBSNEXT_ONLINE_CLASSIC_SUBSCRIPTION_ID_LEGACY );
            }
        }

        return $ebsnext_subscription_id;
    }

    /**
     * get the Ebsnext Online Classic Transaction id from the order
     *
     * @param WC_Order $order
     */
    public static function get_ebsnext_online_classic_transaction_id( $order ) {
        $transaction_id = $order->get_transaction_id();
        //For Legacy
        if( empty( $transaction_id ) ) {
            $order_id = self::is_woocommerce_3() ? $order->get_id() : $order->id;
            $transaction_id = get_post_meta( $order_id, self::EBSNEXT_ONLINE_CLASSIC_TRANSACTION_ID_LEGACY, true );
            if( !empty( $transaction_id ) ) {
                //Transform Legacy to new standards
                delete_post_meta( $order_id, self::EBSNEXT_ONLINE_CLASSIC_TRANSACTION_ID_LEGACY );
     ...
     * @param WC_Order $order
     */
    public static function get_ebsnext_online_classic_callback_url( $order_id ) {
        $args = array( 'wc-api' => 'Ebsnext_Online_Classic', 'wcorderid' => $order_id);
    ...
    /**
     * Create the Ebsnext Online Classic payment html
     *
     * @param mixed $json_data
     * @return string
     */
    public static function create_ebsnext_online_classic_payment_html( $json_data ) {
        $html = '<section>';
        $html .= '<h3>' . __( 'Thank you for using Ebsnext Online ePay.', 'ebsnext-online-classic' ) . '</h3>';
        $html .= '<p>' . __( 'Please wait...', 'ebsnext-online-classic' ) . '</p>';
        $html .= sprintf( '<script type="text/javascript" src="%s" charset="UTF-8"></script>', self::EBSNEXT_instance()->plugin_url( '/scripts/ebsnext-online-classic-payment-window.js' ) );
        $html .= sprintf( '<script type="text/javascript" charset="UTF-8">EbsnextOnlineClassicPaymentWindow.init(%s);</script>', $json_data );
        $html .= doMerchantPayments();
        $html .= '</section>';
        return $html;
    }
     ...
     * @return bool
     */
    public static function validate_ebsnext_online_classic_callback_params( $params, $md5_key, &$order, &$message ) {
    ...
    /**
     * Return the Ebsnext Online Classic instance
     *
     * @return Ebsnext_Online_Classic
     */
    public static function EBSNEXT_instance() {
        return Ebsnext_Online_Classic::get_instance();
    }
    ...
    public static function add_admin_notices($type, $message, $keep_post = false) {
        ...
        $messages = get_option(self::EBSNEXT_ONLINE_CLASSIC_STATUS_MESSAGES, false);
        if(!$messages) {
            update_option(self::EBSNEXT_ONLINE_CLASSIC_STATUS_MESSAGES, array($message));
        } else {
            array_push($messages, $message);
            update_option(self::EBSNEXT_ONLINE_CLASSIC_STATUS_MESSAGES, $messages);
        }
        update_option(self::EBSNEXT_ONLINE_CLASSIC_STATUS_MESSAGES_KEEP_FOR_POST, $keep_post);
    }
    ...
    public static function echo_admin_notices(){
        ...
        foreach($messages as $message) {
            echo Ebsnext_Online_Classic_Helper::message_to_html( $message['type'], $message['message'] );
        }
        if(!get_option( self::EBSNEXT_ONLINE_CLASSIC_STATUS_MESSAGES_KEEP_FOR_POST, false )) {
            delete_option( self::EBSNEXT_ONLINE_CLASSIC_STATUS_MESSAGES );
        } else {
            delete_option( self::EBSNEXT_ONLINE_CLASSIC_STATUS_MESSAGES_KEEP_FOR_POST );
        }
    }
}
