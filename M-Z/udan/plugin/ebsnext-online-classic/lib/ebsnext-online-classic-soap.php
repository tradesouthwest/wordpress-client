 ...
 */
class Ebsnext_Online_Classic_Soap {
	...
	public function __construct( $pwd = '', $subscription = false ) {
		...
		if ( $subscription ) {
			$this->client  = new SoapClient( 'https://dev.znextech.com/nextpay/api/merchant-payments' );
	...
	public function get_epay_error( $merchantnumber, $epay_response_code ) {
		...
		try {
			...
			$epay_params['merchantnumber'] = $merchantnumber;
			$epay_params['pwd'] = $this->pwd;
			$epay_params['language'] = Ebsnext_Online_Classic_Helper::get_language_code( get_locale() );
